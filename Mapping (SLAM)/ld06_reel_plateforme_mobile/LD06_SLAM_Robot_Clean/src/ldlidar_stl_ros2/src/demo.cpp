...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src,  double lidar_spin_freq, LaserScanSetting& setting,
  ...
  if (lidar_spin_freq > 0) {
    ...
    // First fill all the data with infinity to hide invalid points in RViz
    output.ranges.assign(beam_size, std::numeric_limits<float>::infinity());
    output.intensities.assign(beam_size, 0.0f);
    for (auto point : src) {
      ...
      if ((point.distance == 0) && (point.intensity == 0)) { // filter is handled to  0, infinity will be assigned to hide invalid points.
      ...
      if (index < beam_size) {
        ...
        if (setting.laser_scan_dir) {
          ...
          // If the current content is infinity (invalid), assign directly
          // Otherwise, only assign if the new range is less (closer)
          if (std::isinf(output.ranges[index_anticlockwise]) || range < output.ranges[index_anticlockwise]) {
          ...
          // If the current content is infinity (invalid), assign directly
          // Otherwise, only assign if the new range is less (closer)
          if (std::isinf(output.ranges[index]) || range < output.ranges[index]) {
